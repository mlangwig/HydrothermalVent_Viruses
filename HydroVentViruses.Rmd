...
# Modifying github repo

Git rm --cached to remove things from the website. Online, can go to previous commits where file was still there and you can still check the history. Here is an example:

1. Git rm cached what you want
```{bash, eval = FALSE}
git rm --cached ANI_clustering/ANI_clustering.R
```

2. Git commit, push
```{bash, eval = FALSE}
git commit -m "remove ANI_clustering.R"
...
1. Git rm cached what you want
```{bash, eval = FALSE}
git rm -r --cached ANI_clustering/Input/sulfur/
```

2. Git commit, push
```{bash, eval = FALSE}
git commit -m "remove cached clust input sulfur"
