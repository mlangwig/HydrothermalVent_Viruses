...
## Clustering based on blast and python scripts

Trying the pipeline used in [paper](https://github.com/snayfach/MGV/blob/master/ani_cluster/README.md)
...
2. Compute ANI from BLAST results
```{bash, eval = FALSE}

```

3. Perform centroid-based clustering
```{bash, eval = FALSE}
...
4. Compile the CheckV info to provide to dRep: genome,completeness,contamination
...
```{bash, eval = FALSE}
nohup dRep dereplicate -g input_viruses_fna/*fasta --S_algorithm ANImf -nc .5 -l 1000 -N50W 0 -sizeW 1 --ignoreGenomeQuality --clusterAlg single &
```
