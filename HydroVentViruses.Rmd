...
## CheckV 43,995 viruses
```{bash, eval = FALSE}
export CHECKVDB="/storage2/databases/CheckV/checkv-db-v1.5/"

nohup checkv end_to_end ../Virus_Genomes/43995_final_VentViruses_Nlinked.fna CheckV_Output -t 20 &
```
...
## Run iphop on vUnbinned and vMAGs 43,995 viruses
...
5. Finally, run iphop 
```{bash,  eval = FALSE}
nohup iphop predict --fa_file Virus_Genomes/43995_final_VentViruses_Nlinked.fna --db_dir iphop/iPHoP_Aug_2023_w_VentPlumeMAGs/ --out_dir iphop/iphop_43995_VentViruses_custDB -t 60 --no_qc &
```
...
## Rerun seqkit on viruses 43,995
...
2. Rerun Seqkit genome size on all viruses vent and plume, vMAG and vUnbinned, all at once. Doing this on Virserver, 02/18/24. Seqkit Version 2.6.1
```{bash, eval = FALSE}
mamba activate

nohup sh -c 'for i in 43995_VentViruses_individual_fnas/*.fasta ; do seqkit stats $i >> ../../seqkit/PlumeVent_Virus_Seqkit_stats.tsv ; done' &

less PlumeVent_Virus_Seqkit_stats.tsv | sort | uniq | sed 's+43995_VentViruses_individual_fnas/++g' | sed 's/.fasta//g' > PlumeVent_Virus_Seqkit_stats_renamed.tsv
...
```{bash, eval = FALSE}
nohup genomad end-to-end ../Virus_Genomes/43995_final_VentViruses_Nlinked.fna genomad_end /storage2/databases/genomad_db/ -t 20 --splits 5 --cleanup --min-virus-hallmarks-short-seqs 0 &
```
...
python /scratch/langwig/scripts/fix_genomad_taxonomy.py -i 43995_final_VentViruses_Nlinked_taxonomy.tsv -o 43995_final_VentViruses_Nlinked_taxonomy_parsed.tsv
```

3. Get number of hallmarks from genomad annotate output and see how many are ≥1
```{bash, eval = FALSE}
cut -f1,14 43995_final_VentViruses_Nlinked_genes.tsv | sed 's/\(.*\)_/\1\t/' virus_hallmarks.tsv | cut -f1,3 > virus_hallmarks.tsv
...
## Running coverm again with 42,815 viruses

Broke vMAGs with high protein redundancy into individual scaffolds again. Rerunning:
```{bash, eval = FALSE}
pwd
/scratch/langwig/Projects/VentPlumeViruses/abundance/coverm_count_final

mamba activate coverm

find . -type f -name "*.fasta" -print0 | xargs -0 cat > ../final_VentViruses_42833.fna
...
3. Choose the best annotation per scaffold based on bit score and then e value for tie breaker. I completed this in R.
