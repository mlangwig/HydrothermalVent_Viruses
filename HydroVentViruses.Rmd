...
## Run iphop on vUnbinned and vMAGs 42,815 viruses
...
```{bash,  eval = FALSE}
nohup iphop add_to_db --fna_dir MAGs_microbial/fna_noVirusContam/ --gtdb_dir iphop/cleanMAGs_gtdbtk_denovo_wf_2_3_2 --out_dir iphop/iPHoP_Aug_2023_w_VentPlumeMAGs --db_dir /storage2/databases/iPHoP/Aug_2023_pub_rw/ -t 60 &
```

5. Finally, run iphop 
```{bash,  eval = FALSE}
nohup iphop predict --fa_file Virus_Genomes/final_VentViruses_42815.fna --db_dir iphop/iPHoP_Aug_2023_w_VentPlumeMAGs/ --out_dir iphop/iphop_42815_VentViruses -t 60 --no_qc &
```
...
## Rerun seqkit on viruses 42,815
...
2. Rerun Seqkit genome size on all viruses vent and plume, vMAG and vUnbinned, all at once. Doing this on Virserver, 02/13/24. Seqkit Version 2.6.1
```{bash, eval = FALSE}
mamba activate

nohup sh -c 'for i in Virus_Genomes/fna/final_VentVirus_individual_fnas/*.fasta ; do seqkit stats $i >> seqkit/PlumeVent_Virus_Seqkit_stats.tsv ; done' &

less PlumeVent_Virus_Seqkit_stats.tsv | sort | uniq | sed 's+Virus_Genomes/fna/final_VentVirus_individual_fnas/++g' | sed 's/.fasta//g' > PlumeVent_Virus_Seqkit_stats_renamed.tsv

less PlumeVent_Virus_Seqkit_stats_renamed.tsv | tr -s ' ' > PlumeVent_Virus_Seqkit_stats_renamed_space.txt

tail -n +49 PlumeVent_Virus_Seqkit_stats_renamed_space.txt > PlumeVentVirus_Seqkit_final.txt
```
...
## genomad rerun?
**Haven't run this 01/09/24**
Rerunning genomad to see if I can get tax for all viruses
```{bash, eval = FALSE}
mamba activate genomad

nohup genomad annotate Virus_Genomes/vUnbinned_vMAGs_1500Ns_PlumeVent.fna genomad_output/genomad_output_anno genomad_db &
```
